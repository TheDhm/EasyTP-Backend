...
from main.models import DefaultUser, AccessGroup, App, Pod, Instances, UserActivity
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'role', 'group', 'upload_limit', 'size_uploaded',
            'apps_available', 'date_joined', 'is_active'
        ]
...
class PodSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Pod
        fields = [
            'id', 'pod_user', 'pod_user_username', 'app_name', 'pod_name',
            'pod_vnc_user', 'pod_vnc_password', 'date_created', 'date_modified',
            'pod_namespace', 'is_deployed'
        ]
...
class InstanceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Instances
        fields = [
            'id', 'pod', 'pod_details', 'instance_name',
            'date_created', 'date_modified', 'novnc_url'
        ]
...
class UserActivitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserActivity
        fields = [
            'id', 'user', 'user_username', 'user_email', 'username',
            'activity_type', 'activity_display', 'timestamp', 'ip_address',
            'user_agent', 'details'
        ]
...
class SignupSerializer(serializers.ModelSerializer):
    ...
    def validate_username(self, value):
        ...
        if value.lower() in [name.lower() for name in reserved_usernames]:
            raise serializers.ValidationError(
                f"Username '{value}' is reserved and cannot be used."
            )
    ...
    def create(self, validated_data):
        ...
        # Use the provided username (already validated)
