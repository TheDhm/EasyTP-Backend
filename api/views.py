...
from main.models import DefaultUser, App, Pod, Instances, UserActivity, AccessGroup
...
from .serializers import (
    UserSerializer, SignupSerializer, LoginSerializer, AppSerializer,
    PodSerializer, UserActivitySerializer, AppsDataSerializer,
    FileExplorerSerializer, FileUploadSerializer, FileItemSerializer,
    StorageUsageSerializer, FilePermissionsSerializer
)
...
class StartPodView(APIView):
    ...
    def post(self, request, app_name):
        ...
        try:
            ...
            # Update instance
            instance, created = Instances.objects.get_or_create(
                pod=pod,
                instance_name=pod.pod_name
            )
            if novnc_url:
        ...
        try:
            ...
            try:
                ...
                apps_instance.delete_namespaced_deployment(
                    name=f"{app_name_lower}-deployment-{pod_name}",
                    namespace="apps"
                )

                # Delete service
                api_instance.delete_namespaced_service(
                    name=f"{app_name_lower}-service-{pod_name}",
                    namespace="apps"
                )
    ...
    def get(self, request, path=None):
        ...
        if user.role in [DefaultUser.STUDENT, DefaultUser.GUEST]:
            ...
            storage_usage = {
                ...
                'percentage': min(100, (actual_usage / user.upload_limit) * 100) if user.upload_limit > 0 else 0
            }
    ...
    def delete(self, request, path):
        ...
        try:
            ...
            if not os.path.isfile(full_file_path) or os.path.islink(full_file_path):
                return Response({
                    'error': 'Can only delete regular files'
                }, status=status.HTTP_400_BAD_REQUEST)
