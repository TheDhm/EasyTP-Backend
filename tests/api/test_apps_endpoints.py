...
from main.models import DefaultUser, App, Pod, AccessGroup
...
class TestAppsView:
    ...
    def test_student_sees_apps_from_their_group(self, api_client, student_with_app_access, test_app):
        ...
        with patch('api.views.display_apps') as mock_display:
            mock_display.return_value = {test_app.name: {'name': test_app.name, 'status': 'stopped'}}
...
class TestStartPodView:
    ...
    def test_student_with_access_can_start_pod(self, api_client, student_with_app_access, test_app):
        ...
        with patch('api.views.deploy_app') as mock_deploy, \
             patch('api.views.create_service') as mock_service, \
             patch('api.views.create_ingress') as mock_ingress, \
             patch('api.views.stop_deployed_pod') as mock_stop:
...
class TestStopPodView:
    ...
    def test_student_with_access_can_stop_pod(self, api_client, student_with_app_access, test_app):
        ...
        with patch('api.views.load_k8s_config') as mock_config, \
             patch('kubernetes.client') as mock_k8s_client, \
             patch('api.views.delete_ingress') as mock_del_ing:
