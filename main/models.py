...
class DefaultUser(AbstractUser):
    ...
    size_uploaded = models.IntegerField(default=0, help_text=_('Total size uploaded by the user in MB'))    
...
def generate_pods(sender, instance, created, **kwargs):
    if created:
        ...
        if not instance.group:
            ...
            logger.warning(f"User {instance.username} created without group assignment. No pods will be generated.")
            return
...
class UserActivity(models.Model):
    ...
    user = models.ForeignKey('DefaultUser', on_delete=models.SET_NULL, related_name='activities', null=True, blank=True)
