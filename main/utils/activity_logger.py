...
class ActivityLogger:
    ...
    def _is_valid_public_ip(ip_str):
        ...
        try:
            ...
            if ip.version == 6 and not (ip.is_loopback or ip.is_reserved or ip.is_multicast):
                ...
                if not (ip.is_link_local or (int(ip) & 0xfe00000000000000000000000000000000) == 0xfc00000000000000000000000000000000):
                    return True
    ...
    def log_logout(user, request):
        ...
        ActivityLogger.log_activity(
            user=user,
            activity_type=UserActivity.LOGOUT,
            request=request
        )
    ...
    def log_pod_start(user, app_name, pod_name, request=None):
        ...
        ActivityLogger.log_activity(
            user=user,
            activity_type=UserActivity.POD_START,
            request=request,
            details=details
        )
    ...
    def log_pod_stop(user, app_name, pod_name, request=None):
        ...
        ActivityLogger.log_activity(
            user=user,
            activity_type=UserActivity.POD_STOP,
            request=request,
            details=details
        )
