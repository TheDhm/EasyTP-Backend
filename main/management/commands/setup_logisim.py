        ...
        try:
            with transaction.atomic():
                ...
                logisim_app, created = App.objects.get_or_create(
                    name="LOGISIM", defaults={"name": "LOGISIM", "image": "logisim:latest"}
                )

                if created:
                    self.stdout.write(self.style.SUCCESS("Successfully created LOGISIM app"))
                else:
                    self.stdout.write(self.style.WARNING("LOGISIM app already exists"))
